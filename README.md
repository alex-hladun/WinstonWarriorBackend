...
```bash
winston-sam-function$ sam build
```
...
```bash
winston-sam-function$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
winston-sam-function$ sam local start-api
winston-sam-function$ curl http://localhost:3000/
```
...
```bash
winston-sam-function$ sam logs -n HelloWorldFunction --stack-name winston-sam-function --tail
```
...
```bash
winston-sam-function$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name winston-sam-function
```
